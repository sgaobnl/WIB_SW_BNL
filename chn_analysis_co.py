...
if True:
    ...
    for chni in range(128):
        ...
        if plane ==1:
            pus.append([plane, strip, wibi, fembi%4, chn])
            pl = "U"
        if plane ==2:
            pvs.append([plane, strip, wibi, fembi%4, chn])
            pl = "V"
        if plane ==3:
            pxs.append([plane, strip, wibi, fembi%4, chn])
            pl = "X"
...
#    exit()
    ...
    #for x in pxs:
        if x[0] ==1:
            if (x[1] > 90) and (x[1] < 476-90) and ((x[1]-1) not in [324,325, 326,327, 328, 329,330]):
                cos.append(x)
        elif x[0] ==2:
            if (x[1] > 90) and (x[1] < 476-90) and ((x[1]-1) not in [48, 49, 54]):
                cos.append(x)
        elif x[0] ==3:
            if  ((x[1]-1) not in [169, 217, 239]):
                cos.append(x)
    ...
    if cos[0][0] == 1:
        label = "U%d-U%d"%(chnmin, chnmax)
    if cos[0][0] == 2:
        label = "V%d-V%d"%(chnmin, chnmax)
    if cos[0][0] == 3:
        label = "X%d-X%d"%(chnmin, chnmax)
    pi = len(cos)
    for i in range(pi):
        print (cos[i])
        if i == 0:
            chnrmsdata= np.array(rawdata[cos[i][4]])
        else:
            chnrmsdata=chnrmsdata + np.array(rawdata[cos[i][4]])
    print (pi)
    chnrmsdata = chnrmsdata/pi
    chninfo = noise_a_chn(chnrmsdata, chnno=0, fft_en = True, fft_s=2000, fft_avg_cycle=50)
...
#                print ("Large RMS", chx, np.std(rawdata[chx][0:1000]))
    ...
    plt.plot(x[1000:2000], chninfo[3][0:xlen][1000:2000], marker='.',color='r', label = label)
    plt.legend()
#    plt.title ("Waveform @ WIB%dFEMB%dCH%d, %s plane # %d"%(wibi, (fembi%4-1), chi, pl, strip ))
    plt.title ("Waveform of coherent noise")
    plt.xlabel ("Time / us")
    plt.ylabel ("ADC / bit" )
    plt.grid()
    ...
    if xlen > len(chninfo[3]):
        xlen = len(chninfo[3])
    x = (np.arange(xlen))*512/1000.0
    plt.plot(x, chninfo[3][0:xlen], marker='.',color='r', label = label)
    print (np.std(chninfo[3][1000:2000]), np.std(chninfo[3][0:20000]), len(chninfo[3]))
    plt.legend()
#    plt.title ("Waveform @ WIB%dFEMB%dCH%d, %s plane # %d"%(wibi, (fembi%4-1), chi, pl, strip ))
    plt.title ("Waveform of coherent noise")
    plt.xlabel ("Time / us")
    ...
    plt.subplot(223)
    plt.plot(chninfo[4], chninfo[5], marker='.',color='r', label =label )
    plt.legend()
#    plt.title ("FFT @ WIB%dFEMB%dCH%d, %s plane # %d"%(wibi, (fembi%4-1), chi, pl, strip ))
    plt.title ("Spectrum of coherent noise")
...
#    plt.subplot(224)
#    plt.plot(chninfo[6], chninfo[7], marker='.',color='r', label =  "U91-U126")
#    plt.legend()
##    plt.title ("FFT @ WIB%dFEMB%dCH%d, %s plane # %d"%(wibi, (fembi%4-1), chi, pl, strip ))
#    plt.xlim((0,30000))
#    plt.xlabel ("Frequency / Hz")
