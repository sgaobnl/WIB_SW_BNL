...
class plot_tools:
      ...
      def PlotWaveforms(self, data, femb_no, chan): 
          
          pldata = qc.data_decode(data)
      ...
      def PlotAlignment(self, data, fembs, nevent=1): 
          ...
          for ifemb in fembs:
              ...
              for chan in range(128):
                  global_ch = ifemb*128+chan
                  for itr in range(nevent):
                      xx = range(len(pl_data[itr][global_ch][:]))
                      ax[ix,iy].plot(xx,pl_data[itr][global_ch][:])
              ax[ix,iy].set_title('FEMB{} event# {}'.format(ifemb, itr))
      ...
      def GetRMS(self, data, nfemb, chan):
      ...
      def GetGain(self, nfemb, chan, dac_list, snc, sgs, sgp, namepat):
      ...
      def FEMB_CHK_PLOT(self, chn_rmss,chn_peds, chn_pkps, chn_pkns, chn_onewfs, chn_avgwfs):
...
if __name__=='__main__':
   ...
   for i in range(120,127):
       pltools.PlotWaveforms(rawdata, nfemb, i)
